...
function html_inline (tokens, idx, options, env, self) { // eslint-disable-line camelcase
  if (tokens[idx].content === '<kbd>') {
    tokens[idx].tag = 'kbd'
    tokens[idx].attrPush(['class', 'white shadow-5 h1 code f3 br2 bg-dark-gray pa2'])
  }
  if (tokens[idx].content === '</kbd>') {
    tokens[idx].tag = 'kbd'
