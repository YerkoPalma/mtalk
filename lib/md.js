...
function paragraph_open (tokens, idx, options, env, self) { // eslint-disable-line camelcase
  ...
  if (aIndex < 0) {
    tokens[idx].attrPush(['class', 'f4 f3-ns lh-copy georgia'])
  } else {
    tokens[idx].attrs[aIndex][1] = 'f4 f3-ns lh-copy georgia'
  }
...
function fence (tokens, idx, options, env, self) { // eslint-disable-line camelcase
  ...
  if (info) {
    ...
    return '<pre class="bg-mid-gray pa3"><code' + self.renderAttrs(tmpToken) + '>' +
           highlighted +
          '</code></pre>\n'
  }

  return '<pre class="bg-mid-gray pa3"><code' + self.renderAttrs(token) + '>' +
      highlighted +
      '</code></pre>\n'
}
function heading_open (tokens, idx, options, env, self) { // eslint-disable-line camelcase
  ...
  if (level === 1) level = '-headline'
  if (level === 2) level = '-subheadline'
  if (aIndex < 0) {
...
module.exports = function (text) {
  ...
  var rules = {paragraph_open, link_open, image, heading_open, blockquote_open, fence} // eslint-disable-line
